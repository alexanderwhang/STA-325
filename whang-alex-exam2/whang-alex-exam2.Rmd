...
b. Yes, the parameter estimates appear close to the true estimates. Because the algorithm converges successfully after a sufficient amount of iterations, it is ok to say that the mixture model recovers the true parameters well.
...
# Step 2: Calculate initial mixing proportions
cluster_labels <- kmeans_result$cluster
mixing_proportions <- table(cluster_labels) / length(data) # Proportions for each cluster

# Step 3: Calculate initial rate parameters (λ)
rate_parameters <- numeric(K) # Initialize rate parameters
for (k in 1:K) {
  cluster_data <- data[cluster_labels == k] # Data points in cluster k
  rate_parameters[k] <- 1 / mean(cluster_data) # Estimate rate as reciprocal of mean
}

# Print initial parameters
cat("Initial Mixing Proportions:", mixing_proportions, "\n")
cat("Initial Rate Parameters:", rate_parameters, "\n")

exponentialMixture <- function(data, K, max_iter = 1000, tol = 1e-5,
                                init_pi = NULL, init_lambda = NULL) {
  n <- length(data)
  pi <- if (is.null(init_pi)) rep(1/K, K) else init_pi # Initialize mixing proportions
  lambda <- if (is.null(init_lambda)) runif(K, 0.1, 1) else init_lambda # Initialize rate parameters
  log_likelihoods <- numeric(max_iter) # Store log-likelihood values
  
  for (iter in 1:max_iter) {
    # E-step: Compute posterior probabilities (gammas)
    gamma <- matrix(NA, nrow = n, ncol = K)
    for (k in 1:K) {
      gamma[, k] <- pi[k] * dexp(data, rate = lambda[k])
    }
    row_sums <- rowSums(gamma) # Normalize probabilities
...
# Run EM with k-means initialization
